...
class ShiftController():
    def add(self,datetime, cook, oficiant_1, oficiant_2):
        ...
        oficiant_id = UserController.show(oficiant_1)
        oficiant_2_id = UserController.show(oficiant_2)
        print(type(cook_id.id))
        print(cook_id.id)
        Shifts.create(datetime = datetime, cook_id = cook_id.id, oficiant_id = oficiant_id.id, oficiant_2_id = oficiant_2_id.id)
